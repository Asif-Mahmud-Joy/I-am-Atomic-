...
module.exports = {
  config: {
    ...
    longDescription: {
      en: "Recommend anime based on genre using Jikan API or fallback local data. Use: {prefix} anime [genre] [number]"
    },
    category: "Entertainment",
    guide: "{prefix} anime [genre] [1-5]"
  },
  ...
  onStart: async function ({ api, args, message, event }) {
    const animeRecommendations = {
            shonen: [
                ...
                },
                {
                    ...
                    imageUrl: "https://drive.google.com/uc?export=download&id=1oRD7AAH_VD73o8kUlUaJQC1dFTrV1nDz"
                },
                {
                    animeName: "Dr. Stone",
    ...
    const { lastRecommendations } = this;

    if (args.length === 0) {
      const genreList = Object.keys(animeRecommendations).join(', ');
      return message.reply(`🎌 Bhai, genre de na! Available genres: ${genreList}`);
    }
    ...
    if (args.length > 1 && !isNaN(args[1])) {
      ...
      if (number < 1 || number > 5) {
        return message.reply("⚠️ Number 1-5 er moddhe hote hobe, bhai!");
      }
    }

    if (!(genre in animeRecommendations)) {
      return message.reply("⚠️ Genre ta nai! Valid genres: shonen, seinen, isekai, scifi");
    }

    const loadingMessage = await api.sendMessage("⏳ Anime load hocche, ekto wait kor...", event.threadID);

    try {
      const genreMap = { shonen: 1, seinen: 42, isekai: 62, scifi: 24 };
      const response = await axios.get(`https://api.jikan.moe/v4/anime?genres=${genreMap[genre]}&limit=50`);

      const apiAnimes = response.data.data.map(anime => ({
        animeName: anime.title,
        imageUrl: anime.images.jpg.image_url
      }));

      const combined = [...apiAnimes, ...animeRecommendations[genre]];
      if (!lastRecommendations[genre]) lastRecommendations[genre] = [];

      const selected = [];
      while (selected.length < number && combined.length > 0) {
        const i = Math.floor(Math.random() * combined.length);
        if (!lastRecommendations[genre].includes(i)) {
          selected.push(combined[i]);
          lastRecommendations[genre].push(i);
          if (lastRecommendations[genre].length > 10) lastRecommendations[genre].shift();
        }
      }

      for (const anime of selected) {
        const stream = await global.utils.getStreamFromURL(anime.imageUrl);
        await api.sendMessage({
          body: `🎌 Genre '${genre}' er jonno recommendation: **${anime.animeName}**\nDekhle moja pabi!`,
          attachment: stream
        }, event.threadID);
      }
    } catch (err) {
      const fallback = animeRecommendations[genre];
      const selected = [];
      while (selected.length < number && fallback.length > 0) {
      ...
      for (const anime of selected) {
        const stream = await global.utils.getStreamFromURL(anime.imageUrl);
        await api.sendMessage({
          body: `📡 API down, fallback use kore '${genre}' genre er jonno: **${anime.animeName}**`,
          attachment: stream
        }, event.threadID);
      }
    }
