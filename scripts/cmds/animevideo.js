...
module.exports = {
  config: {
    ...
    countDown: 15,
    role: 0,
    shortDescription: "Anime video dekhte chai? Ai command use koro",
    longDescription: "Random anime edit video pathabe, real working & no API issue",
    category: "anime",
    guide: "{pn} animevdo"
  },
  ...
  onStart: async function ({ api, event, message }) {
    ...
    const loading = await message.reply("⏳ Random anime video khuja hocche...");
    ...
    try {
      ...
      await message.reply({ body: "✅ ENJOY KORO! 🤍", attachment: vidStream });
    } catch (err) {
      console.error("Video load error:", err);
      await message.reply("❌ Video pathano jayna, onno ta try koro!");
    }
