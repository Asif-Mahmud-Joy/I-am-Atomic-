module.exports = {
    config: {
        ...
        countDown: 5,
        role: 0,
        shortDescription: "Send a random girl image",
        longDescription: "Sends a random image of a girl from a predefined list.",
        category: "media",
        guide: "{pn}"
    },

    // Complete list of image links provided by the user
    linkList: [
    ...
    getRandomImage: async function(linkList, maxRetries = 3) {
        for (let i = 0; i < maxRetries; i++) {
            const img = linkList[Math.floor(Math.random() * linkList.length)];
            try {
                const stream = await global.utils.getStreamFromURL(img);
                return stream;
            } catch (e) {
                console.log(`Failed to fetch image: ${img}`);
            }
        }
        return null;
    },
    ...
    onStart: async function ({ message }) {
        const stream = await this.getRandomImage(this.linkList);
        if (stream) {
            message.send({
                body: 'Here is your girl image 😻 / এখানে আপনার মেয়ের ছবি 😻',
                attachment: stream
            });
        } else {
            message.send('Sorry, I could not fetch a valid image after several tries. Please try again later.');
        }
    }
};
